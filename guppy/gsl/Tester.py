class Doc2Tester:
    ...
    def get_condition_method(self, cond):
        ...
        if name is None:
            ...
            ap(f'def {name}({argstring}):', '<NL>', '<INDENT>')
            ap(f'# Condition: {idn}', '<NL>')
            ...
            for pc in pcs:
                for ic in pc.find_aspects('in_context'):
                    ...
                    for line in ctx.split('\n'):
                        ap(line, '<NL>')
                if pc is pcs[-1]:
                    ap(f'return ({pc.src.node.arg.strip()})', '<NL>')
                else:
                    ap(f'if not ({pc.src.node.arg.strip()}):', '<NL>', '<INDENT>')
    ...
    def get_children(self, node):
        return (
            (self.doc.node_of_taci('name', node.arg.strip()),)
            + tuple(node.children)
            if node.arg.strip()
            else node.children
        )
    ...
    def get_condition_expr(self, po, args, attr, src, tgt):
        ...
        return f"{'', 'not '[po.is_not]}self.{name}({', '.join(callargs)})"
    ...
    def get_example_expr(self, eg):
        ...
        if ex is not None:
            return ex
        ex = eg.get_ex_text()
        if ctx := eg.get_ctx_text():
            ex_name = 'get_ex_%d' % self.ex_id
            self.ex_id += 1
            self.ap(f'def {ex_name}(self):', '<NL>', '<INDENT>')
            for line in ctx.split('\n'):
                self.ap(line, '<NL>')
            self.ap(f'return {ex}', '<NL>', '<DEDENT>')
            ex = f'self.{ex_name}()'
    ...
    def get_tester(self):
        ap = self.ap
        ap(f'# Tests generated by: {__name__}', '<NL>')
        # ap('# Main source file: %s' %
        #    self.doc.env.get_package().get_filename(), '<NL>')

        ap(f'# Date: {self.mod.time.asctime(self.mod.time.localtime())}', '<NL>')
    ...
    def visit_test_program_segment(self, node):
        self.gen_comment(f'Test for: {node.arg.strip()}')
        self._visit_children(node)
...
class DiffKind:
    ...
    def get_id_name(self):
        return f'{self.a.get_id_name()}+{self.b.get_id_name()}' + "'"
...
class TestOf(Doc2Tester):
    ...
    def make_cat(self):
            ...
            if tag == 'attribute':
                ...
                if otherkinds or not atkak:
                    if len(otherkinds) != len(atkak):
                        ...
                        atk.tgtfullname = f"({'&'.join([x.tgtfullname for x in otherkinds])})"
        ...
        def assign_fa(append, tgt, func, src):
            ...
            if tag == 'getattr':
                name = func[1]
                append(f'{tgt} = {self.getattr_code(src, name)}')
                ...
                if sus is not None:
                    for su in sus:
                        append(f'{src}.{su.get_name()}({su.get_args()})')
                else:
                ...
                for eq in op.find_equations():
                    ...
                    if len(pres) != 1:
                        node = pres[0].src.node if pres else eq.src.node
                ...
                if tag == 'callattr':
                    append(f'{tgt} = {self.getattr_code(src, opname)}({args})')
                elif tag == 'operator':
                    append(f'{tgt} = {src} {opname} {args}')
                elif tag == 'inplace_operator':
                    append(f'{tgt} = {src}')
                    append(f'{tgt} {opname} {args}')
                elif tag == 'reverse_operator':
                    append(f'{tgt} = {args} {opname} {src}')
                elif tag == 'function_operator':
                    argstr = str(args)
                    if argstr:
                        argstr = f', {argstr}'
                    append(f'{tgt} = {opname}({src}{argstr})')
                elif tag == 'mapping':
                    append(f'{tgt} = {src}({args})')
                elif tag == 'getitem':
                    # Number of args >=1, has been checked.
                    append(f'{tgt} = {src}[{args}]')
                elif tag == 'delitem':
                    # Number of args >=1, has been checked.
                    append(f'del {src}[{args}]')
                elif tag == 'setitem':
                    # Number of args >=2, has been checked.
                    append(f"{src}[{', '.join(args.negs[:-1])}] = {args.negs[-1]}")
                else:
                    assert 0
        ...
        class ArrowTree:
            ...
            def __str__(self):
                chdrn = f', chdrn = {self.children}' if self.children else ''
                return f'AT(pre = {self.pre}{chdrn})'
        ...
        ap(f'def {test_name}(self, arg):', '<NL>', '<INDENT>')
...
class _GLUECLAMP_:
    ...
    def doc2filer(self, doc, node, name, dir, opts, IO):
        ...
        path = IO.path.join(dir, f'{name}.py')
